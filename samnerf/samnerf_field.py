...
class SAMNERFField(Field):
    ...
    def get_outputs(self, ray_samples: RaySamples, clip_scales) -> Dict[SAMNERFFieldHeadNames, TensorType]:
        ...
        outputs[SAMNERFFieldHeadNames.HASHGRID] = x.view(*ray_samples.frustums.shape, -1)

        clip_pass = self.clip_net(torch.cat([x, clip_scales.view(-1, 1)], dim=-1)).view(*ray_samples.frustums.shape, -1)
        outputs[SAMNERFFieldHeadNames.CLIP] = clip_pass / clip_pass.norm(dim=-1, keepdim=True)

        dino_pass = self.dino_net(x).view(*ray_samples.frustums.shape, -1)
        outputs[SAMNERFFieldHeadNames.DINO] = dino_pass
